...
import { heartIcon, starIcon } from "@/packages/configs/config.images";
...
const SVGIcon: React.FC<IconProps> = ({
  ...
  const gradientOffset = percentage <= 0 ? 0 : percentage >= 100 ? 511 : (percentage * 512) / 100;
...
const RatingIcon: React.FC<IconRenderProps> = ({
  ...
  return (
    <svg
      ...
      fill="none"
    >
      <path d={svgPath} fill={`url(#${gradientId})`} stroke={fillColor} strokeWidth={10} />
      <defs>
        <linearGradient id={gradientId} x1={gradientOffset} gradientUnits="userSpaceOnUse">
...
// Popup Variant
const PopupIcon: React.FC<IconRenderProps> = ({ gradientOffset, fillColor, svgPath, svgSize }) => {
  ...
  return (
    <>
      ...
      <svg viewBox="0 0 512 512" width={svgSize} height={svgSize} style={{ display: "none" }}>
        <defs>
